...
grunt.initConfig({
	...
	sass: {
		dev: {
			...
			files: {
				"<%= project.build %>/assets/styles/app.css": "<%= project.css %>/app.scss"
			}
		},
		build: {
			options: {
...
jade: {
	...
	autoprefixer: {
		...
		target: {
			files: {
				"<%= project.build %>/assets/styles/app.css": "<%= project.build %>/assets/styles/app.css"
			}
		}
	},
	// JS Error checking
	...
	copy: {
		main: {
			expand: true,
			cwd: '<%= project.app %>/',
			src: ['content/**', 'scripts/vendor/*.js','assets/images/**', 'assets/javascripts/**', 'assets/fonts/**', 'robots.txt'],
			dest: '<%= project.build %>/',
		}
	},
	sync: {
